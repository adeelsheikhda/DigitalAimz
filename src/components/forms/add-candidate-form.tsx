...
export function AddCandidateForm() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', phone: '', position: '', resume_text: '',
  })
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true)
    setError('')
    const res = await fetch('/api/data/candidates', {
      method: 'POST',
    ...
    setLoading(false)
    if (data.success) {
      setForm({ name: '', email: '', phone: '', position: '', resume_text: '' })
      setOpen(false)
      router.refresh()
    } else {
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 overflow-hidden">
      ...
      >
        <span className="flex items-center gap-2 font-medium">
          <span className="text-blue-400">＋</span> Add New Candidate
        </span>
      ...
      {open && (
        <form onSubmit={submit} className="border-t border-zinc-800 p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-zinc-500 text-xs mb-1 block">Position *</label>
              <input
                required value={form.position} onChange={e => set('position', e.target.value)}
          ...
          <div>
            <label className="text-zinc-500 text-xs mb-1 block">Resume / CV Text *</label>
            <textarea
              required value={form.resume_text} onChange={e => set('resume_text', e.target.value)}
              placeholder="Paste the full resume text here — the HR agent will read and score this..."
              rows={5}
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              ...
              className="flex-1 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-500 rounded-lg transition-all disabled:opacity-50"
            >
